...
class FaceDetectionTranslator(
    ...
    override fun postProcessOutput(input: BufferedImage, output: Map<String, Tensor>): List<DetectedBiometric> {
        ...
        val scores = output["scores"]!!.data.rewind().array() as FloatArray //TODO("check !!")
        val boxes = output["boxes"]!!.data.rewind().array() as FloatArray
        val scoresSize = scores.size / 2
        val boxesSize = boxes.size / 4
        val scoresReshaped = OrtUtil.reshape(scores, longArrayOf(scoresSize.toLong(), 2)) as Array<FloatArray>
        val boxesReshaped = OrtUtil.reshape(boxes, longArrayOf(boxesSize.toLong(), 4)) as Array<FloatArray>
